...
class FactorAnalysisDaysTimeHorizonSimple():
    """ 
    输入factor，ret_dict
    ...
    def run(self):
        ...
        for adate in (self.dateint_list):
...
def updateFactorAnalysisResSimple(config_path):
    ...
    def analysis_parallel(factor_name,FACTOR_path):
        ...
        faDays = FactorAnalysisDaysTimeHorizonSimple(
            factor = FACTOR,
    ...
    Parallel(n_jobs=N_CPU)(delayed(analysis_parallel)(factor_name, FACTOR_path) for factor_name,FACTOR_path in zip(factor_name_list_all,factor_path_list_all))
