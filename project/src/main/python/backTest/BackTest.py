...
from factorAnalysis.factorAnalysisCalTools import (FactorDescribe, neutralize_method_dict,
                                    standardize_method_dict,time_horizon_dict,
                                    BARRA_FACTOR_NAME_LIST)
from factorAnalysis.factorAnalysisIOTools import (load_stock_code_trade_date, load_stock_pool,
                                   loadmat2df_factor, loadmat2df_interday,
                                   save_this,loadpkl2df_factor,load_obj_pkl,load_hx_feature)
from factorAnalysis.factorAnalysisSingleDay import FactorAnalysisSingleDay
...
class BackTest():
    ...
    def __init__(self,factor,factor_style_dict,stock_pool,ret_dict,benchmark_return_df,industry_name_dict=None,factor_pool_dict=None,corr_test_date_list=None,group_num=10,time_horizon_dict=time_horizon_dict):
        self.factor = factor
    ...
    def run(self):
        ...
        for adate in tqdm(self.dateint_list):
            ...
            IC_th_dict[time_horizon].append(fasd.calculate_IC(ret=ret_neutral,method='spearman',filter_arr=lmt_filter_adate_arr))
            IC_th_all_barra_dict[time_horizon].append(fasd_all_barra.calculate_IC(ret=ret_neutral,method='spearman',filter_arr=lmt_filter_adate_arr))
            ...
            group_IC_th_list_dict[time_horizon].append(fasd.calculate_group_IC_rank(factor_rank=factor_rank_adate,ret_rank=ret_rank_adate,group_weight_oneday=group_weight_oneday_eqW,method='spearman',filter_arr=lmt_filter_adate_arr))
            # group_IC_th_list_dict[time_horizon].append(fasd.calculate_group_IC(group_weight_oneday=group_weight_oneday_eqW,ret=ret_neutral,method='spearman',filter_arr=lmt_filter_adate_arr))
        ...
        self.IC_th_df = pd.DataFrame(IC_th_dict["RET_t2"],index=self.dateint_list,columns=["RET_t2"])
        self.IC_th_all_barra_df = pd.DataFrame(IC_th_all_barra_dict["RET_t2"],index=self.dateint_list,columns=["RET_t2"])
        self.factor_return_th_df = pd.DataFrame(factor_return_th_dict["RET_t2"],index=self.dateint_list,columns=["RET_t2"])

        # self.factor_pool_corr_df = pd.DataFrame(factor_pool_corr_dict["RET_t2"],index=factor_pool_corr_date_list)
        ...
        self.group_return_eqW_th_dict = {"RET_t2":pd.DataFrame(group_return_eqW_th_list_dict['RET_t2'],index=self.dateint_list,columns=group_cols_list)[group_cols_list_order]}
        ...
        self.group_excess_return_eqW_th_dict = {"RET_t2":(self.group_return_eqW_th_dict["RET_t2"]).sub(self.benchmark_return_eqW_th_df["RET_t2"],axis=0)}
        ...
        self.group_IC_th_dict = {"RET_t2":pd.DataFrame(group_IC_th_list_dict["RET_t2"],index=self.dateint_list,columns=group_cols_list)[group_cols_list_order]}
        self.long_group = group_cols_list_order[-1]
        self.short_group = group_cols_list_order[0]
        self.long_short_return_eqW_th_df = pd.DataFrame({"RET_t2":self.group_return_eqW_th_dict["RET_t2"][self.long_group].sub(self.group_return_eqW_th_dict["RET_t2"][self.short_group])})
...
def updateFactorAnalysisRes(config_path):
    ...
    def analysis_parallel(factor_name,FACTOR_path):
        ...
        FACTOR = load_hx_feature(FACTOR_path).reindex(columns=stock_code_cols).dropna(how='all').loc[config['start_date']:config['end_date']].astype(float)
        faDays = BackTest(
            factor = FACTOR,
